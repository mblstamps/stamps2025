...
library(dada2); packageVersion("dada2") # 1.36 or later
...
# The following forward and reverse primers  were used to amplify the full-length 16S gene
FWD <- "AGRGTTYGATYMTGGCTCAG" # The 27F primer
REV <- "RGYTACCTTGTTACGACTT"  # The 1492R primer
...
prim <- removePrimers(fns, nops, 
                      primer.fwd=XXX primer.rev=YYY, # REPLACE XXX/YYY with proper parameter choices
...
# If almost no reads passed primer removal -- did you provide the right parameters?
# In particular, take a look at `?removePrimers` -- what does it say about primer.rev?
# Note: The dada2 function `rc` will reverse complement a DNA sequence.
